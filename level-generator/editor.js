...
class LevelEditor {
    ...
    resize() {
        ...
        if (newWidth >= 5 && newWidth <= 30 && newHeight >= 5 && newHeight <= 20) {
            this.width = newWidth;
    ...
    canPushBlock(blockX, blockY, dx) {
        ...
        if (this.grid[blockY][newX] === 'wall') return false;

        // Check block collision
        if (this.blocks.has(`${newX},${blockY}`)) return false;

        // Check if there's space above (player needs to fit)
        if (this.grid[blockY - 1]?.[blockX] === 'wall') return false;
    ...
    async loadLevelsList() {
        try {
            ...
            levels.forEach(level => {
                ...
                li.textContent = `#${level.id}: ${level.name}`;
